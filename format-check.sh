...
if [[ $(basename $PWD) != "bulldogs-tp" ]]; then
  echo "Must run from bulldogs-tp directory."
  exit 1
fi
...
fail() {
  ...
  echo "Example:"
  echo " $ cd bulldogs-tp"
  echo " $ tar -cf project2-lastname-firstname.tar Sender.py README.txt"
  echo "---------------------------------------------------------------"
...
python2 TestHarness.py -s FormatCheckSender.py -r Receiver.py
rm FormatCheckSender.py
